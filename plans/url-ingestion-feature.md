...
- `URL` model: `id, url (unique), title, fetched_at, created_at`
- `URLChunk` model: `id, content, content_hash (unique), url_id (FK), chunk_order, is_summary, embedding (Vector 1536), created_at`
- Unified response models: `SourceResponse`, `ContentItemResponse`, `ContentWithRelatedItemsResponse`
...
**1.3 Add Repository Interfaces to `src/repositories/interfaces.py`:**
...
**Note:** Add `count_with_embeddings()` to both NoteRepository and URLChunkRepository for efficient random selection (counts rows where embedding IS NOT NULL)

### Phase 2: Repository Implementations

**2.1 Create `src/repositories/url_repository.py`:**
- Pattern: Mirror `BookRepository` structure
- Key feature: Deduplication by URL in `add()` method

**2.2 Create `src/repositories/urlchunk_repository.py`:**
...
### Phase 3: URL Fetching & Content Processing

**3.1 Create `src/url_fetcher.py`:**
...
**3.2 Create `src/content_chunker.py`:**
...
**3.3 Create `src/url_processor.py`:**
...
### Phase 4: Unified Random Endpoint

**4.1 Create `src/random_selector.py`:**
...
**4.2 Update `src/routers/response_builders.py`:**
...
### Phase 5: URL-Specific Endpoints

**5.1 Create `src/routers/urls.py`:**
...
**6.1 Update `src/routers/search.py`:**
...
**7.1 Update `src/dependencies.py`:**
...
**7.3 Update `src/test_utils.py`:**
- Add: `StubURLRepository`, `StubURLChunkRepository`

### Phase 8: Testing

**8.1 Unit Tests (create new files):**
...
1. **Phase 1** (Models & Migration) - Run migration to create tables
2. **Phase 2** (Repositories) - Data access layer
3. **Phase 3** (Processing) - URL fetching, chunking, processing
4. **Phase 4** (Unified /random) - Update existing endpoint
5. **Phase 5** (URL Endpoints) - New API surface
6. **Phase 6** (Search) - Enhanced search
7. **Phase 7** (DI & Config) - Wire everything together
8. **Phase 8** (Testing) - Ensure quality throughout
...
*Last Updated: Confirmed user decisions - synchronous processing, return existing URL, 500KB size limit, skip evaluation for URL chunks*
