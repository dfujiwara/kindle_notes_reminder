...
def test_random_v2_missing_book_returns_500(setup_random_v2_deps: RandomV2DepsSetup):
    """Test GET /random/v2 returns 404 when note's book is missing."""
    ...
    # Should get 404 because book doesn't exist
    assert response.status_code == 404
    data = response.json()
    assert "Book not found" in data["detail"]


def test_random_v2_missing_url_returns_404(setup_random_v2_deps: RandomV2DepsSetup):
    """Test GET /random/v2 returns 404 when chunk's URL is missing."""
    ...
    # Should get 404 because URL doesn't exist
    assert response.status_code == 404
    data = response.json()
    assert "URL not found" in data["detail"]
