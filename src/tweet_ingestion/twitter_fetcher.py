...
def _empty_tweet_list() -> "list[FetchedTweet]":
    return []
...
class FetchedTweet:
    ...
    content: str
    media_urls: list[str] = field(default_factory=_empty_str_list)
...
class FetchedThread:
    ...
    author_username: str
    author_display_name: str
    tweets: list["FetchedTweet"] = field(default_factory=_empty_tweet_list)
...
def parse_tweet_url(url: str) -> str | None:
    ...
    Returns:
        Tweet ID if valid URL, None otherwise
    """
    match = TWEET_URL_PATTERN.match(url)
...
def parse_tweet_input(input_str: str) -> str:
    ...
    tweet_id = parse_tweet_url(input_str)
    if tweet_id:
        return tweet_id
